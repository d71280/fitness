-- フィットネススタジオ予約システム完全スキーマ
-- 既存のPrismaスキーマをSupabaseに移行
...
CREATE TABLE instructors (
  ...
  email VARCHAR(255),
  phone VARCHAR(50),
  specialties TEXT[], -- PostgreSQLの配列型
  bio TEXT,
...
CREATE TABLE studios (
  ...
  equipment TEXT[], -- PostgreSQLの配列型
  description TEXT,
  operating_hours JSONB, -- PostgreSQLのJSON型
  is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE customers (
  ...
  preferred_programs INTEGER[], -- プログラムIDの配列

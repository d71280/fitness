...
export default function SettingsPage() {
  ...
  const testSimpleSheets = async () => {
    setLoading(true)
    try {
      console.log('シンプルなGoogle Sheets書き込みテストを開始...')
      
      const response = await fetch('/api/test-simple-sheets', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        }
      ...
      if (!response.ok) {
        const errorText = await response.text()
        console.error('シンプルテストAPI応答エラー:', errorText)
        alert(`❌ シンプルテストAPI呼び出し失敗 (${response.status}):\n\n${errorText}`)
        return
      }
      ...
      alert(`シンプルテストでエラーが発生しました: ${error instanceof Error ? error.message : '不明なエラー'}`)
    } finally {
      setLoading(false)
    }
  }
          ...
          {googleSheetsSettings.enabled && (
            <>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <Button
                  onClick={testSimpleSheets}
                  disabled={loading}
                  variant="outline"
                  className="w-full sm:w-auto bg-purple-50 hover:bg-purple-100"
                >
                  <TestTube className="h-4 w-4 mr-2" />
                  🚀 シンプルテスト
                </Button>
